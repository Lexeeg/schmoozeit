...
export async function PUT(
  ...
  const inProgressRaw = body?.inProgress;
  const inProgress =
    typeof inProgressRaw === "boolean" ? inProgressRaw : null;

  const supabase = getSupabaseAdmin();
  const { error } = await supabase
    .from("submissions")
    .update({ admin_in_progress: inProgress })
    .eq("id", id);

  if (error) {
    return NextResponse.json(
      { error: "Failed to save in progress", details: error.message },
      { status: 500 },
    );
  }
